...
float dtbToanTruong[100]; // M?ng luu di?m trung bình t?ng h?p c?a t?ng SV
char hbToanTruong[100];   // M?ng luu lo?i h?c b?ng (A, B, C) c?a t?ng SV

void docDuLieuTichLuy(char dsMaSV[][20], char dsTenSV[][50], float dsCPA[], int soMonHocCuaSV[], int *tongSoSV) {
    int i, k, m;
    *tongSoSV = 0;

    // Kh?i t?o m?ng d? li?u tích luy t? danh sách g?c c?a h? th?ng
    for (i = 0; i < n; i++) {
    ...
    // Ð?c t?t c? các file h?c ph?n d? c?ng d?n di?m cho t?ng SV
    for (m = 0; m < soHp; m++) {
        char fileMon[50];
    ...
    // Tính di?m trung bình tích luy CPA th?c t?
...
void tinhToanHocBong(float dsCPA[], int soMonHocCuaSV[], int tongSoSV, 
                     long long tienHBDuKienCaNhan[], char loaiHBCaNhan[], char xlTichLuyCaNhan[],
                     long long *tongHocPhiToanLop, long long *quyHocBongToiDa) {
    long long hocPhiGocMotMon = 3000000; 
    long long tongTienHBDuKien = 0;
    int i;

    *tongHocPhiToanLop = 0;
    for (i = 0; i < tongSoSV; i++) {
        *tongHocPhiToanLop += (soMonHocCuaSV[i] * hocPhiGocMotMon);
    }
    *quyHocBongToiDa = *tongHocPhiToanLop * 0.08;

    // Xét duy?t danh hi?u h?c b?ng lý thuy?t ban d?u
    for (i = 0; i < tongSoSV; i++) {
        loaiHBCaNhan[i] = 'C';
        xlTichLuyCaNhan[i] = ' ';
        tienHBDuKienCaNhan[i] = 0;

        if (soMonHocCuaSV[i] > 0) {
            xlTichLuyCaNhan[i] = xepLoai(dsCPA[i]);
            if (xlTichLuyCaNhan[i] == 'A' && dsCPA[i] > 9.0) {
                loaiHBCaNhan[i] = 'A';
                tienHBDuKienCaNhan[i] = soMonHocCuaSV[i] * hocPhiGocMotMon * 1.5;
            } 
            else if ((xlTichLuyCaNhan[i] == 'A' && dsCPA[i] <= 9.0) || (xlTichLuyCaNhan[i] == 'B' && dsCPA[i] > 8.0)) {
                loaiHBCaNhan[i] = 'B';
                tienHBDuKienCaNhan[i] = soMonHocCuaSV[i] * hocPhiGocMotMon * 1.2;
            }
        }
        tongTienHBDuKien += tienHBDuKienCaNhan[i];
    }

    // Áp d?ng thu?t toán cân d?i qu? th?c t? (C?t gi?m thông minh)
    double heSoDieuChinhK = 1.0;
    if (tongTienHBDuKien > *quyHocBongToiDa && *quyHocBongToiDa > 0) {
        heSoDieuChinhK = (double)*quyHocBongToiDa / tongTienHBDuKien;
        
        // C?p nh?t l?i s? ti?n th?c nh?n tr?c ti?p trên m?ng d? ki?n
        for (i = 0; i < tongSoSV; i++) {
            tienHBDuKienCaNhan[i] = (long long)(tienHBDuKienCaNhan[i] * heSoDieuChinhK);
        }
    }
}

int demKyTuHienThi(const char *str) {
    int len = 0;
    while (*str) {
        // Trong UTF-8, các byte ti?p theo c?a 1 ký t? có d?u d?u có d?ng 10xxxxxx (nh? hon -64 ho?c t? 128 d?n 191)
        if ((*str & 0xC0) != 0x80) {
            len++;
        }
        str++;
...
void InBangDiemToanLop() {
    ...
    // Khai báo các c?u trúc d? li?u luu tr? c?c b?
    char dsMaSV[100][20];
    char dsTenSV[100][50];
    float dsCPA[100];
    ...
    long long tienHocBongThucTe[100] = {0};
    char loaiHBCaNhan[100];
    char xlTichLuyCaNhan[100];
    
    long long tongHocPhiToanLop = 0;
    long long quyHocBongToiDa = 0;
    long long tongTienHBThucTePhat = 0;
    int i, j, k;

    // 1. G?i hàm x? lý t?ng d? li?u di?m
    docDuLieuTichLuy(dsMaSV, dsTenSV, dsCPA, soMonHocCuaSV, &tongSoSV);

    // 2. G?i hàm x? lý nghi?p v? cân d?i tài chính h?c b?ng
    tinhToanHocBong(dsCPA, soMonHocCuaSV, tongSoSV, tienHocBongThucTe, loaiHBCaNhan, xlTichLuyCaNhan, &tongHocPhiToanLop, &quyHocBongToiDa);

    // 3. S?p x?p th? h?ng SV theo di?m CPA gi?m d?n
    for (i = 0; i < tongSoSV - 1; i++) {
        for (j = i + 1; j < tongSoSV; j++) {
            if (dsCPA[i] < dsCPA[j]) {
                ...
                long long tempTien = tienHocBongThucTe[i]; tienHocBongThucTe[i] = tienHocBongThucTe[j]; tienHocBongThucTe[j] = tempTien;
                char tempLoaiHB = loaiHBCaNhan[i]; loaiHBCaNhan[i] = loaiHBCaNhan[j]; loaiHBCaNhan[j] = tempLoaiHB;
                char tempXl = xlTichLuyCaNhan[i]; xlTichLuyCaNhan[i] = xlTichLuyCaNhan[j]; xlTichLuyCaNhan[j] = tempXl;
            }
        }
    }

    // 4. In giao di?n b?ng d? li?u (Ðã c? d?nh d? r?ng khung)
    printCenter(GREEN "-------------------- BANG DIEM TICH LUY & THONG KE HOC BONG TOAN LOP --------------------" RESET);
    printf("\n");
    printf("+------+-----------+---------------------+---------------+------------+---------+------------------+\n");
    printf("| HANG |   MSSV    |       Ho Va Ten     | DIEM TICH LUY | SO MON HOC | LOAI HB | SO TIEN HOC BONG |\n");
    printf("+------+-----------+---------------------+---------------+------------+---------+------------------+\n");
    
    for (i = 0; i < tongSoSV; i++) {
        tongTienHBThucTePhat += tienHocBongThucTe[i];

        // In H?ng và MSSV c? d?nh
        printf("|  %-3d | %-9s | ", i + 1, dsMaSV[i]);

        // X? lý in Tên SV và bù kho?ng tr?ng thông minh ch?ng l?ch ti?ng Vi?t
        printf("%s", dsTenSV[i]);
        int thucTeTen = demKyTuHienThi(dsTenSV[i]);
        for (k = 0; k < (19 - thucTeTen); k++) {
            printf(" ");
        }
        printf(" | ");

        // C?t Ði?m Tích Luy kèm màu s?c (Ðã tách mã màu ra bên ngoài rìa, không làm l?ch l?)
        if (loaiHBCaNhan[i] == 'A' || loaiHBCaNhan[i] == 'B') {
            printf(GREEN);
            printf("%-5.2f (%c)", dsCPA[i], xlTichLuyCaNhan[i]);
            printf(RESET);
            printf("     | ");
            ...
            if (soMonHocCuaSV[i] > 0) printf("%-5.2f (%c)     | ", dsCPA[i], xlTichLuyCaNhan[i]);
            else printf("0.00 ( )      | ");
        }

        printf("  %-8d | ", soMonHocCuaSV[i]);

        // C?t Danh hi?u lo?i h?c b?ng kèm màu s?c
        if (loaiHBCaNhan[i] == 'A' || loaiHBCaNhan[i] == 'B') {
            printf("   " GREEN "%c" RESET "    | ", loaiHBCaNhan[i]);
        } else {
            printf("   %c    | ", loaiHBCaNhan[i]);
        }

        // C?t S? ti?n nh?n th?c t? kèm màu s?c
        if (tienHocBongThucTe[i] > 0) {
            printf(GREEN "%11lld VND" RESET "  | \n", tienHocBongThucTe[i]);
        } else {
            printf("%11lld VND  | \n", tienHocBongThucTe[i]);
        }
    }
    printf("+------+-----------+---------------------+---------------+------------+---------+------------------+\n");
	printf("| TONG HOC PHI THU DUOC TOAN LOP       : " GREEN "%15lld VND" RESET "                                       |\n", tongHocPhiToanLop);
    printf("| QUY HOC BONG DUOC TRICH (8%% TONG HP) : " GREEN "%15lld VND" RESET "                                       |\n", quyHocBongToiDa);
    printf("| TONG CHI TRA THUC TE SAU CAN DOI QUY  : " GREEN "%15lld VND" RESET "                                      |\n", tongTienHBThucTePhat);
    printf("+--------------------------------------------------------------------------------------------------+\n");

    printf("\n" YELLOW "Nhan phim bat ky de quay lai menu chon hoc phan..." RESET);
    getch();
}
...
int MenuIn() {
    ...
    while (1) {
        ...
        // In danh sách các môn h?c t? d?ng (1, 2, 3...)
        for (i = 0; i < soHp; i++) {
            sprintf(line, "|   %2d. %-33s   |", i + 1, dsHocPhan[i]);
            printCenter(line);
        }
        printCenter("|-------------------------------------------|");
        
        // M?c t?ng h?p t? d?ng x?p ? cu?i danh sách môn h?c
        sprintf(line, "|   %2d. Xem Bang Diem & Hoc Bong Toan Lop   |", soHp + 1);
        printCenter(line);
        ...
        if (chon == 0) return 0; // Quay l?i menu qu?n lý chính
        
        // N?u ch?n trúng s? (soHp + 1): Vào th?ng b?ng di?m & h?c b?ng tích luy
        if (chon == soHp + 1) {
            InBangDiemToanLop(); 
            continue; // Xem xong quay l?i menu ch?n môn
        }
        
        // N?u ch?n trúng các s? th? t? môn h?c thông thu?ng
        if (chon >= 1 && chon <= soHp) {
            strcpy(tenHp, dsHocPhan[chon - 1]);
            taoTenFile();
            docFile(); 
...
printf("\n");
            printCenter(MAGENTA "--- CONG CU TUONG TAC BANG DIEM MON HOC ---" RESET);
            printCenter("+---------------------------------------------------------+");
            printCenter("| 1: Loc Loai A  | 2: Loc Loai B  | 3: Loc Loai C          |");
            printCenter("| 4: Loc Loai D  | 5: Loc Loai F                           |");
            printCenter("| 7: Reset ve mac dinh                                    |");
            printCenter("| 0: Quay lai menu chon hoc phan                          |"); 
            printCenter("+---------------------------------------------------------+");
            printf("\n");

    ...
    while (1) {
        ...
        for (i = 0; i < soHpCuaLop; i++) {
            ...
            sprintf(file,
                    "%s_%s.txt",
                    danhSachLop[lopHienTai].tenLop,
                    danhSachLop[lopHienTai].dsHocPhan[i]);
            FILE *f = fopen(file, "r");
            int found = 0;
            ...
            int laMonSua =
                (strcmp(danhSachLop[lopHienTai].dsHocPhan[i], monSua) == 0);
            if (found) {
...
void ThongKeHocVu(HocKy *ky) {
    ...
    for (i = 0; i < ky->soLuongLop; i++) {
        ...
        for (j = 0; j < lop->soLuongHocPhan; j++) {
            ...
            printf(BLUE   "C: %.1f%%   " RESET, demC * 100.0 / coDiem);
...
void In(HocPhan *hp, char *tenLopSinhHoat) {
    ThongKeHocVu();
    int loaiLoc = 0;
    while (1) {
        int demA = 0, demB = 0, demC = 0, demD = 0, demF = 0;
    ...
    while (1) {
        ...
        switch (chon) {
            ...
            case 2: {
                ...
                for (i = 0; i < 2; i++) {
                    char dong[100];
                    sprintf(dong, "|  %d. %-35s |", i + 1, dsHocKy[i].tenKy);
                    printCenter(dong);
                }
                printCenter(CYAN "+-------------------------------------------+" RESET);
                printf("\n");
                ...
                LopHocPhan *lopSV = &ky->dsLopTrongKy[chonLop - 1];
                if (dangNhapSinhVien(lopSV)) {
                    char dsTenHP[10][10];
                    for (i = 0; i < lopSV->soLuongHocPhan; i++) {
                        strcpy(dsTenHP[i],
                               lopSV->dsHocPhan[i].maHP);
                    }
                    HocPhan *hp = &lopSV->dsHocPhan[0];
                    menuSinhVien(svDangNhap, hp, lopSV->tenLop, lopSV->soLuongHocPhan, dsTenHP );
                }
                break;
            }
            case 0: {
